...
namespace MidtierApi
{
    public class Echo
    {
        ...
        public Echo(IAuthToken authToken, IHttpClient httpClient, IConfiguration configuration)
        {
        ...
        public async Task<OkObjectResult> Run(
            ...
            ILogger log)
        {
            log.LogInformation("Midtier Echo HTTP trigger function processed a request.");

            await OnExecutingAsync(req, context);

            var response = await _httpClient.GetAsync($"{_backendUrl}/api/echo/{message}");
